...
def moveToNextCell():
    ...
    print(cellsToWall)
    targetDis = max(cellsToWall - 1, 0) * cellLength + cellDisFront
...
def calibrateGyroFront():
    ...
    print(minAng)
    print(maxAng)
    print((minAng + maxAng) / 2)
    print(getGyro())
    print(targetAngle)'''
    
    correction = 0#-3 if minAng + maxAng > 0 else 3

    setGyro(targetAngle + getGyro() - (minAng + maxAng) / 2 + correction)

'''def calibrateGyroTurret():
    spinTurretToPos(270)
    lastDis = getUltrasonicTurret()
    thisDis = getUltrasonicTurret()

    while thisDis <= lastDis:
        tankDrive(spDriveSlow - 10, -spDriveSlow + 10)
        thisDis = getUltrasonicTurret()
